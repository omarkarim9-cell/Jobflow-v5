...
app.post('/api/test/profile', async (req, res) => {
  try {
    ...
    const email = req.body?.email || (clerkUser && clerkUser.primaryEmailAddress && clerkUser.primaryEmailAddress.emailAddress) || null;
    const full_name = req.body?.full_name || (clerkUser && (clerkUser.fullName || clerkUser.firstName)) || null;
    const resume_content = req.body?.resume_content || null;
    const preferences = req.body?.preferences ? JSON.stringify(req.body.preferences) : null;
    const connected_accounts = req.body?.connected_accounts ? JSON.stringify(req.body.connected_accounts) : null;
    const plan = req.body?.plan || null;
    const phone = req.body?.phone || null;
    const resume_file_name = req.body?.resume_file_name || null;
